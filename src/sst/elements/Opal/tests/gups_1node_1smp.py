...
for next_core in range(cores):
	...
	cpuMMULink = sst.Link("cpu_mmu_link_" + str(next_core))
	...
	if next_core < cores//2:
	        comp_cpu = sst.Component("cpu"+str(next_core), "miranda.BaseCPU")
        	comp_cpu.addParams({
                	"verbose" : 1,
	        })
	        ...
	        cpugen.addParams({
        	        "verbose" : 0,
                	"count" : 10000,
	                "max_address" : ((shared_memory_capacity) // 2) * 1024,
        	})
		comp_cpu.enableAllStatistics({"type":"sst.AccumulatorStatistic"})

                cpuMMULink.connect((comp_cpu, "cache_link", "300ps"), (mmu, "cpu_to_mmu%d"%next_core, "300ps"))
